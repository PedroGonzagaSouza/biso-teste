...
#     return {"login": currentUser["sub"]}33
...
async def getCurrentUser(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    try:
        ...
        if payload is None:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token inválido")
        # user = db.query(Usuario).filter(Usuario.LOGIN == payload["sub"]).first()
...
async def getProfile(currentUser: UsuariosRequest = Depends(getCurrentUser)):
    return {
...
async def login(usuario: UsuariosRequest, db: Session = Depends(get_db)):
    try:
        ...
        return {"access_token": accessToken, "token_type": "bearer", "user": user.LOGIN, "id": user.USERID, "nome": user.NOME}
    except Exception as e:
        raise e
